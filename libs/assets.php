...
function cssTags(){
	...
	if($config['env'] == 'prod'){
		$css_tags = '<link rel="stylesheet" type="text/css" href="'.$config['address'].'/assets/css/master'.$css_md5.'.css" media="screen">';
	}else{
...
function jsTags(){
	global $config, $js_files, $js_md5;
	$js_tags = '';

	if($config['env'] == 'prod'){
		$js_tags = '<script type="text/javascript" src="'.$config['address'].'/assets/js/master'.$js_md5.'.js"></script>';
	}else{
